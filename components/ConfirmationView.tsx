...
const ConfirmationView: React.FC<ConfirmationViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center animate-fade-in">
      <div className="bg-navy-light p-10 rounded-lg shadow-2xl max-w-2xl mx-auto">
        ...
        <h1 className="text-4xl font-bold text-white mb-2">{t('thank_you', '¡Gracias por tu compra!')}</h1>
        <p className="text-lg text-metal-300 mb-6">{t('order_confirmation', 'Tu pedido ha sido realizado con éxito. Hemos enviado una confirmación a tu correo electrónico.')}</p>
        <div className="space-y-4 md:space-y-0 md:space-x-4">
          <button
            onClick={() => alert(t('invoice_alert', 'Función de facturación simulada. ¡La factura se ha descargado!'))}
            className="bg-brand-secondary text-white font-bold py-3 px-6 rounded-md hover:bg-blue-800 transition-colors"
          >
            {t('download_invoice', 'Descargar Factura')}
          </button>
          <button
            onClick={() => onNavigate('home')}
            className="bg-brand-primary text-white font-bold py-3 px-6 rounded-md hover:bg-orange-500 transition-colors"
          >
