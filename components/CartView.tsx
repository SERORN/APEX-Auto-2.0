...
const CartView: React.FC<CartViewProps> = ({ onNavigate }) => {
  ...
  // Mostrar precios con incremento del 10%
  const subtotal = state.items.reduce((sum, item) => sum + Math.round(item.price * 1.10 * 100) / 100 * item.quantity, 0);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-background rounded-lg shadow-md border border-border-color p-6 space-y-4">
            {state.items.map(item => {
              const finalPrice = Math.round(item.price * 1.10 * 100) / 100;
              return (
                <div key={item.id} className="flex items-start sm:items-center gap-4 border-b border-border-color pb-4 last:border-b-0 flex-col sm:flex-row">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-lg font-semibold text-brand-primary sm:hidden mt-1">{formatCurrency(finalPrice)}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      aria-label={t('cart_quantity_for', `Cantidad para ${item.name}`)}
                    />
                    <button onClick={() => handleRemoveItem(item.id)} className="text-text-secondary hover:text-[#E53E3E] p-1 rounded-full" aria-label={t('cart_remove', 'Eliminar del carrito')}>
                      <XIcon className="w-6 h-6" />
                    </button>
                  </div>
                  <div className="w-24 text-right hidden sm:block">
                    <p className="text-lg font-bold text-text-primary">{formatCurrency(finalPrice * item.quantity)}</p>
                  </div>
                </div>
              );
            })}
